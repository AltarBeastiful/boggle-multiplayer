...
import grammalecteSource from 'dictionary-fr';
 ...
 * and elided forms out: none of them is all lower-case letters.
 */
...
function grammalecteForms() {
  const utf8 = new TextDecoder();
  const lines = utf8.decode(grammalecteSource.dic).split('\n');
  ...
  for (let index = 1; index < lines.length; index++) {
    ...
    for (let at = 0; at + 1 < flags.length; at += 2) {
      ...
      if (!UNPLAYABLE_FLAGS.has(flag)) playableFlags.push(flag);
    }
    const word = line.slice(0, slash);
    kept.push(playableFlags.length > 0 ? `${word}/${playableFlags.join('')}` : word);
  }

  // The reader takes files, so the trimmed copy is written beside the sources
  // it was made from rather than kept in memory.
  mkdirSync(WORK, { recursive: true });
  const dic = resolve(WORK, 'grammalecte-fr.dic');
  const aff = resolve(WORK, 'grammalecte-fr.aff');
  writeFileSync(dic, `${kept.length}\n${kept.join('\n')}\n`, 'utf8');
  writeFileSync(aff, utf8.decode(grammalecteSource.aff), 'utf8');
  return { dic, aff, entries: kept.length };
}

const grammalecte = grammalecteForms();
const reader = await IterableHunspellReader.createFromFiles(grammalecte.aff, grammalecte.dic);
const curated = [];
let expanded = 0;
for (const word of reader) {
  expanded++;
  if (FRENCH.test(word)) curated.push(word);
}
console.log(`\nGrammalecte: ${grammalecte.entries} entries, ${expanded} forms expanded`);
console.log(`  ${curated.length} of them lower-case French`);
const blockGrammalecte = admit(curated);
